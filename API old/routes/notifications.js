...
function getNotifications(req, res, next) {
    ...
    let token = req.body.token || req.headers['token'],
        ...
        input = {};
    input.id = decoded.id || 0;
    input.limit = req.headers["limit"] || req.headers["size"] || 10;
    input.offset = req.headers["offset"] || ((req.headers["limit"] || req.headers["size"]) ? req.headers["page"]*limit : 0);
    // console.log(input.id);
    // Get notifications
    pgp.db.any(`
        SELECT id, message, link, icon, read --, createdAt
        FROM notifications
